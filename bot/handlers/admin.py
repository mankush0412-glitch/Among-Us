...
async def endgame_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "No winner. Use /startgame to start a new game.",
        parse_mode="Markdown"
    )
...
async def addtask_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            "📝 *Example:*\n`/addtask Who was the first person to walk on the moon?`\n\n"
            "✅ The task will appear in future games for this group!",
            parse_mode="Markdown"
        )
        return
    ...
    await update.message.reply_text(
        f"✅ *Custom task added!*\n\n"
        f"📋 *Task:* {task_text}\n\n"
        f"ℹ️ Use /listtasks to see all custom tasks for this group.",
        parse_mode="Markdown"
    )
...
async def listtasks_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"📋 *Custom Tasks for this group ({len(tasks)}):*\n\n"
    for i, t in enumerate(tasks, 1):
        text += f"`{i}.` {t['task_text']}\n"
        if i >= 20:
            text += f"\n_(+{len(tasks)-20} more)_"
            break
...
async def settime_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• 🏆 Reveal: 9:00 PM\n"
        "• 📊 Scoreboard posted: Every 6 hours\n\n"
        "To change timings, update `.env` and restart the bot.",
        parse_mode="Markdown"
    )
...
async def kickplayer_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("❌ Usage: /kickplayer @username")
        return
    ...
    await update.message.reply_text(
        f"👢 **@{target_username}** has been kicked from the game by admin.",
        parse_mode="Markdown"
    )
...
async def pingall_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = " ".join(context.args) if context.args else "Hey! Don't forget to check the latest task!"
    await update.message.reply_text(
