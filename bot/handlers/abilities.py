...
async def kill_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("❌ Usage: `/kill @username`", parse_mode="Markdown")
        return
    ...
    if not result["success"]:
        if result.get("shielded"):
            await update.message.reply_text(
                f"🛡️ *Kill blocked!* @{target_username}'s shield absorbed the attack!\n"
    ...
    if player.get("last_sabotage"):
        try:
            ...
            if diff < Config.SABOTAGE_COOLDOWN_HOURS:
                ...
                await update.message.reply_text(f"⏰ Sabotage on cooldown! Wait *{remaining}h* more.", parse_mode="Markdown")
                return
        except Exception:
            pass
...
async def scan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("❌ Usage: `/scan @username`", parse_mode="Markdown")
        return
    ...
    hints_sus = [
        f"🔴 @{target_username} was spotted near the vents...",
        f"🔴 @{target_username} completed a task 3x faster than possible.",
        f"🔴 @{target_username}'s biometrics show elevated stress levels.",
        f"🔴 @{target_username} was in a room with no assigned task.",
    ]
    hints_clear = [
        f"🟢 @{target_username} appears to be working diligently on tasks.",
        f"🟢 @{target_username}'s activity log is consistent with a crewmate.",
        f"🟢 @{target_username} was spotted completing the MedBay scan.",
        f"🟢 @{target_username} shows no signs of deception.",
    ]
...
async def shield_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    max_shields = Config.SHIELD_USES + (1 if player.get("is_premium") else 0)
...
async def report_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await chat.send_message(
        ...
        f"Use /vote @player or tap a button below.\n"
...
async def watch_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("❌ Usage: `/watch @username`", parse_mode="Markdown")
        return
    ...
    if user_data and user_data.get("chat_id"):
        ...
        await send_dm(
            context.bot, user_data["chat_id"],
            f"👁️ *Watch Report — @{target_username}*\n\n"
