...
def task_msg(task_text: str, category: str, task_num: int):
    ...
    return (
        ...
        f"{'─' * 28}\n\n"
        f"{task_text}\n\n"
        f"⚡ First correct answer wins +{Config.POINTS_TASK_COMPLETE} pts!\n"
...
def voting_msg(phase: int):
    return (
        ...
        f"Use /vote @player to cast your vote, or tap a button below.\n"
...
def eject_msg(player_name: str, was_imposter: bool, role: str):
    if was_imposter:
        return (
            f"🚀 *{player_name}* was ejected...\n\n"
            f"🔴 *{player_name} WAS the Impostor!*\n\n"
        ...
        return (
            f"🚀 *{player_name}* was ejected...\n\n"
            f"🟢 *{player_name} was NOT the Impostor.*\n\n"
...
def kill_announcement(killer_name: str, victim_name: str):
    return (
        f"💀 *Body Found!*\n\n"
        f"*{victim_name}* has been eliminated.\n"
        f"👻 {victim_name} is now a ghost.\n\n"
...
def game_over_imposter_wins(imposter_name: str):
    return (
        f"😈 *Impostor Wins!*\n"
        f"{'─' * 28}\n\n"
        f"🎭 *{imposter_name}* was the Impostor — and outsmarted everyone!\n\n"
...
def game_over_crew_wins(imposter_name: str):
    return (
        f"🎉 *Crewmates Win!*\n"
        f"{'─' * 28}\n\n"
        f"😈 *{imposter_name}* was the Impostor — but couldn't fool the crew!\n\n"
...
def help_msg():
    return (
        ...
        f"• /kill @player — Eliminate a crewmate\n"
        ...
        f"• /scan @player — Get a sus hint\n"
        f"• /shield — Protect yourself from one kill\n"
        f"• /report — Call an emergency meeting\n"
        f"• /watch @player — Spy on someone\n\n"
        f"*Voting*\n"
        f"• /vote @player — Vote to eject\n"
