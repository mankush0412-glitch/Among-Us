...
async def resolve_target(update, context, db, args):
    """
    Resolve a target player from three sources (in priority order):
      1. A reply to someone's message  — works with no args
      2. A numeric user ID argument    — e.g. /kill 123456789
      3. A @username argument          — e.g. /kill @john or /kill john
    ...
    """
    # ── Priority 1: replied-to message (only when no explicit arg given) ──
