...
class Database:
    ...
    async def update_group_setting(self, group_id: int, key: str, value):
        """Save a single setting key for the group."""
        await self.db.groups.update_one(
            {"group_id": group_id},
    ...
    async def update_group_last_task_at(self, group_id: int):
        """Record the timestamp when a task was last posted to this group."""
        await self.db.groups.update_one(
            {"group_id": group_id},
    ...
    async def get_group_cfg(self, group_id: int) -> dict:
        ...
        stored per-group settings override the global Config defaults.
        """
        defaults = {
            ...
            "kill_cooldown":       Config.KILL_COOLDOWN_HOURS,
            "sabotage_cooldown":   Config.SABOTAGE_COOLDOWN_HOURS,
            ...
            "task_interval":       Config.TASK_INTERVAL_HOURS,
            "voting_hour":         Config.VOTING_START_HOUR,
            "reveal_hour":         Config.REVEAL_HOUR,
            "score_interval":      Config.SCORE_INTERVAL_HOURS,
        }
    ...
    async def list_custom_tasks(self, group_id: int):
        return await self.get_custom_tasks(group_id)
